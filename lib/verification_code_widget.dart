...
class VerificationCodeInputState extends State<VerificationCodeInput> {
  ...
  Widget build(BuildContext context) {
    return TextField(
      ...
      inputFormatters: [LengthLimitingTextInputFormatter(widget.codeLength)],
...
class CustomUnderlineInputBorder extends InputBorder {
  ...
  double get offsetX => textTrueWidth * 0.2;
  ...
  void paint(
    ...
    Path path = Path();
    path.moveTo(rect.bottomLeft.dx + startOffset-offsetX, rect.bottomLeft.dy);
    ...
    path = dashPath.dashPath(path,
        dashArray: dashPath.CircularIntervalList<double>([
          textTrueWidth+offsetX*2,
          letterSpace-offsetX*2,
        ]));
    canvas.drawPath(path, borderSide.toPaint());
  }
}
